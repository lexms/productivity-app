...
import { Calendar, Clock, CheckCircle2, Lightbulb } from "lucide-react"
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {lastCheckin && (
          <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-green-800">Last check-in</span>
            </div>
            <div className="text-right">
              ...
              <div className="text-xs text-green-600">
                Energy: {lastCheckin.energy[0]}/10 • Mood: {lastCheckin.mood[0]}/10
              </div>
            </div>
          </div>
        )}
        ...
        <Button
          ...
          disabled={!shouldShow}
        >
          {shouldShow ? `Complete ${currentTime} check-in` : "Check-in completed recently"}
        </Button>
        ...
        <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-blue-800">Optimal for {currentTime}</span>
          </div>
        ...
        {checkinHistory.length > 0 && (
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-slate-700">Today's Check-ins</h4>
            <div className="grid grid-cols-2 gap-2">
              {checkinHistory.slice(-4).map((checkin, index) => (
                <div key={index} className="p-2 bg-slate-50 rounded text-xs">
                  <div className="font-medium capitalize">{checkin.timeOfDay}</div>
                  <div className="text-slate-600">
                    {new Date(checkin.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                  </div>
                  <div className="text-slate-600">
                    E:{checkin.energy[0]} M:{checkin.mood[0]} F:{checkin.focus[0]}
                  </div>
                </div>
              ))}
            </div>
